...
Load — PDFs are loaded page by page with PyPDFLoader
Split — Pages split with RecursiveCharacterTextSplitter (200-char sliding window)
Embed — Each chunk converted to a 384-dim vector via all-MiniLM-L6-v2
Store — Vectors persisted to ChromaDB on disk
Retrieve — MMR fetches 20 candidates, returns 3 most diverse
Generate — Local Llama3 answers using only retrieved context
Cite — Answer includes source document and page number
